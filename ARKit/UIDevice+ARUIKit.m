...
+ (NSString *)ar_devicePlatformString {
    NSString *platform = [self ar_devicePlatform];
    // iPhone
...
+ (BOOL)ar_isiPad {
    return [[[self ar_devicePlatform] substringToIndex:4] isEqualToString:@"iPad"];
}

+ (BOOL)ar_isiPhone {
    return [[[self ar_devicePlatform] substringToIndex:6] isEqualToString:@"iPhone"];
}

+ (BOOL)ar_isiPod {
    return [[[self ar_devicePlatform] substringToIndex:4] isEqualToString:@"iPod"];
}

+ (BOOL)ar_isSimulator {
    return [[self ar_devicePlatform] isEqualToString:@"i386"]
    || [[self ar_devicePlatform] isEqualToString:@"x86_64"];
}
...
+ (NSString *)ar_uniqueIdentifier {
    ...
    if([[UIDevice currentDevice] respondsToSelector:@selector(identifierForVendor)]) {
        ...
        uuid = [defaults objectForKey:ARUniqueIdentifierDefaultsKey];
        if (!uuid) {
            uuid = [self _generateUUID];
            [defaults setObject:uuid forKey:ARUniqueIdentifierDefaultsKey];
